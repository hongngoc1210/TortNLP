"""
ModernBERT baseline for COLIEE Japanese Tort Prediction (TP).
...
Input schema expected by data_utils:
    undisputed_facts:    list[dict], each dict has "description"
    plaintiff_claims:   list[dict], each dict has "description", "is_accepted"
    defendant_claims:   list[dict], each dict has "description", "is_accepted"
    court_decision:     bool/int or None
    tort_id:            str

Task:
    case-level binary classification: court_decision / tort affirmed.
"""
...
def format_tp_input(sample: dict) -> str:
    """Convert one normalized case from data_utils into one ModernBERT input."""
    p_lines = [f"P{i}: {text}" for i, text in enumerate(sample.get("P", []))]
    d_lines = [f"D{i}: {text}" for i, text in enumerate(sample.get("D", []))]
...
class TPClassificationDataset(torch.utils.data.Dataset):
    def __init__(self, examples: List[TPExample], tokenizer, max_length: int):
...
def load_tp_examples(jsonl_path: str) -> List[TPExample]:
    """
    Build TP examples from the existing MTL normalized dataset.
    Cases with missing court_decision are skipped.
    """
    samples = build_dataset(jsonl_path)
    examples: List[TPExample] = []
    ...
    if not examples:
        raise ValueError(f"No TP training examples with valid T labels found in: {jsonl_path}")
    return examples
...
def build_training_args(args, train_dataset_size: int) -> TrainingArguments:
    """Handle both old and new transformers naming, and avoid deprecated warmup_ratio."""
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/modernbert_tp_baseline")
...
def main():
    ...
    train_ds = TPClassificationDataset(train_examples, tokenizer, args.max_length)
    dev_ds = TPClassificationDataset(dev_examples, tokenizer, args.max_length)
